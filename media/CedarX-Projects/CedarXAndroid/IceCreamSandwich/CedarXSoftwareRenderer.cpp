...
#if ((CEDARX_ANDROID_VERSION == 8) || (CEDARX_ANDROID_VERSION == 9) || (CEDARX_ANDROID_VERSION == 10) || (CEDARX_ANDROID_VERSION == 11))
...
int CedarXSoftwareRenderer::dequeueFrame(ANativeWindowBufferCedarXWrapper *pObject)
{
    ...
    //yaosen
	#if 0
    if(pObject->is_hwc_progressive)
    	native_window_set_usage(mNativeWindow.get(),pANativeWindowBuffer->usage | GRALLOC_USAGE_HW_DEINTERLACE_RENDER);
    #endif

#if (CEDARX_ANDROID_VERSION == 11)
    // dequeue a buffer
    int fenceFd = -1;
    if ((err = mNativeWindow->dequeueBuffer(mNativeWindow.get(), &buf, &fenceFd)) != 0) {
               LOGW("Surface::dequeueBuffer returned error %d", err);
        return -1;
    }
    // wait for the buffer
    sp<Fence> fence(new Fence(fenceFd));
       if ((err = fence->wait(Fence::TIMEOUT_NEVER)) != 0) {
        mNativeWindow->cancelBuffer(mNativeWindow.get(), buf, fenceFd);
               LOGW("fence->wait returned error %d", err);
        return -1;
    }

#elif ((CEDARX_ANDROID_VERSION == 8) || (CEDARX_ANDROID_VERSION == 9)|| (CEDARX_ANDROID_VERSION == 10))
    if ((err = mNativeWindow->dequeueBuffer_DEPRECATED(mNativeWindow.get(), &buf)) != 0) {
        LOGW("Surface::dequeueBuffer returned error %d", err);
        return -1;
    }
...
//    data += src_c_size;
//    memcpy(dst, data, src_display_c_size);
#elif (defined(__CHIP_VERSION_F33) || defined(__CHIP_VERSION_F50) || defined(__CHIP_VERSION_F67))
