...
void CedarXPlayer::finishAsyncPrepare_l(int err){
	...
	if (mVideoWidth && mVideoHeight) {
		// set mediaPlayerInfo
		mExtendMember->mediaPlayerInfo.width = mMediaInfo.mVideoInfo[0].mFrameWidth;
		mExtendMember->mediaPlayerInfo.height = mMediaInfo.mVideoInfo[0].mFrameHeight;
		mExtendMember->mediaPlayerInfo.codecType = mMediaInfo.mVideoInfo[0].mCodecType;
		...
		memset(&mExtendMember->mediaPlayerInfo, 0, sizeof(mExtendMember->mediaPlayerInfo));
...
status_t CedarXPlayer::getMediaPlayerInfo( struct MediaPlayerInfo* mediaPlayerInfo)
{
    LOGV("(f:%s, l:%d)", __FUNCTION__, __LINE__);
	
	mediaPlayerInfo->width = mExtendMember->mediaPlayerInfo.width;
	mediaPlayerInfo->height = mExtendMember->mediaPlayerInfo.height;
	mediaPlayerInfo->codecType = mExtendMember->mediaPlayerInfo.codecType;
...
//	}
...
#if (defined(__CHIP_VERSION_F23) || (defined(__CHIP_VERSION_F51))|| defined(__CHIP_VERSION_F50) || defined(__CHIP_VERSION_F67))
	...
	LOGI("init Render, notify video size %d X %d", nAppVideoWidth, nAppVideoHeight);
...
//			StagefrightVideoRenderExit();
	...
	set3DMode(_3d_mode, display_3d_mode);
		    ...
		    {
            #if (defined(__CHIP_VERSION_F23) || defined(__CHIP_VERSION_F51) || defined(__CHIP_VERSION_F33) || defined(__CHIP_VERSION_F50) || defined(__CHIP_VERSION_F67))//a31 temp change to this, because of vertical screen
                mRenderToDE = 0;
		        mVideoRenderer = new CedarXLocalRenderer(mNativeWindow, meta);
            #elif (defined(__CHIP_VERSION_F33))
                if (0 == mNativeWindow->perform(mNativeWindow.get(), NATIVE_WINDOW_GETPARAMETER,NATIVE_WINDOW_CMD_GET_SURFACE_TEXTURE_TYPE, 0)
...
//			StagefrightVideoRenderExit();
		...
		else
		{
			...
			pANativeWindowBuffer->is_hwc_progressive = frm_inf->is_hwc_progressive;
